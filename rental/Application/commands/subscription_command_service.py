...
class SubscriptionCommandService:
    ...
    def create(self, plan_id: int, negocio_id: int, status: SubscriptionStatus = SubscriptionStatus.ACTIVE) -> Subscription:
        if plan_id is None or negocio_id is None:
            raise ValueError("plan_id and negocio_id are required.")
    ...
    def update(self, subscription_id: int, plan_id: int, negocio_id: int, initial_date: str, final_date: str, status: SubscriptionStatus) -> Subscription:
        subscription = self.subscription_repo.get_by_id(subscription_id)
        if not subscription:
            raise ValueError("Subscription not found")
        subscription.plan_id = plan_id
        subscription.negocio_id = negocio_id
